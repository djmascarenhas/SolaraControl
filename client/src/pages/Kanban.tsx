  ...
  return (
    <Layout>
      <div className="h-full flex flex-col">
        <div className="px-3 sm:px-6 py-3 border-b flex items-center gap-2 sm:gap-4 bg-background/50 backdrop-blur-sm shrink-0 overflow-x-auto">
          <div className="flex items-center gap-2 text-sm text-muted-foreground mr-1 sm:mr-2 shrink-0">
            <Filter className="w-4 h-4" />
            <span className="hidden sm:inline">{t("kanban.filters")}</span>
          </div>
        ...
        <div className="flex-1 overflow-auto p-2 sm:p-3 sm:px-4">
          <div className="flex h-full gap-2 overflow-x-auto pb-2 snap-x md:snap-none min-h-0">
            {statusColumns.map(column => {
              const columnTickets = getTicketsByStatus(column.id);
              
              return (
                <div key={column.id} className="flex-shrink-0 w-[75vw] sm:w-[45vw] md:w-auto md:flex-1 min-w-0 flex flex-col h-full rounded-lg bg-muted/30 border border-border/50 snap-center">
